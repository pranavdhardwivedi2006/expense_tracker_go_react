# 💰 ExpenseTrack - Full Stack Expense Manager

A powerful and responsive **Expense Tracking Application** built to help users manage their finances, visualize spending habits, and stay within budget.

This project leverages the **performance of Go (Golang)** for the backend and the **interactivity of React** for the frontend, ensuring a seamless user experience.

---

## 🚀 Tech Stack

### **Backend**
* **Language:** Go (Golang)
* **Framework:** Gorilla Mux (for Routing)
* **Database:** MongoDB (Atlas)
* **Authentication:** JWT (JSON Web Tokens)
* **Driver:** Official MongoDB Go Driver

### **Frontend**
* **Framework:** React.js (Vite)
* **Styling:** Tailwind CSS
* **Icons:** Lucide React
* **Charts:** Recharts
* **State Management:** React Hooks

---

## ✨ Key Features

* **🔐 Secure Authentication:** User registration and login using JWT.
* **📊 Interactive Dashboard:** Visual breakdown of expenses using Pie Charts and Progress Bars.
* **💰 Budget Management:** Set and update monthly budget limits.
* **📝 CRUD Operations:** Add, Edit, and Delete expenses easily.
* **🔍 Smart Filtering:** Filter transaction history by **Month, Year, and Category**.
* **🌙 Dark/Light Mode:** Fully responsive UI with seamless theme toggling.
* **⚡ High Performance:** Backend utilizes MongoDB Aggregation pipelines for fast data summaries.

---

## 📸 Screenshots

| Dashboard | Add Expense |
|-----------|-------------|
| <img width="1920" height="1080" alt="Screenshot 2026-01-18 233221" src="https://github.com/user-attachments/assets/e839b150-d4d4-47d5-b3b3-9ec875c2fbf9" /> |<img width="1920" height="1080" alt="Screenshot 2026-01-18 233227" src="https://github.com/user-attachments/assets/b2d36942-ec99-4d54-b8dd-890e606ae373" />|

| History & Filters | Settings |
|-------------------|----------|
| <img width="1920" height="1080" alt="Screenshot 2026-01-18 233231" src="https://github.com/user-attachments/assets/0645f1df-1cce-49bf-be1f-c11f74e60959" /> | <img width="1920" height="1080" alt="Screenshot 2026-01-18 233236" src="https://github.com/user-attachments/assets/a096bd6b-ea27-46ba-aec4-82164b92bdd2" /> |

---

## 🛠️ Installation & Setup

Follow these steps to run the project locally.

### 1. Clone the Repository
```bash
git clone [https://github.com/pranavdhardwivedi2006/expense-tracker.git](https://github.com/pranavdhardwivedi2006/expense-tracker.git)
cd expense-tracker
